    ...
    private:
        ...
        public:
            ...
            [[nodiscard]] Element_t top() const {
                ...
                for (std::size_t Index = sizeof(Element_t) / 8; Index < 2 * sizeof(Element_t) / 8; Index++) {
            ...
            void add(Element_t Value) {
                ...
                while (!empty() && top() < Value) {
                    for (std::size_t Index = Current; Index < Current + sizeof(Element_t) / 8; Index++) {
                        NewHead[Index] = Head[Index - Current + sizeof(Element_t) / 8];
                        if (Index + sizeof(Element_t) / 8 < B / 8) {
                            Head[Index - Current + sizeof(Element_t) / 8] = Head[Index + sizeof(Element_t) / 8];
                        }
                    }
        ...
        [[nodiscard]] uint64_t Height(std::size_t X) const {
            ...
            if (X < 1 + 2 + 4 + 8 + 16 + 32 + 128 + 512) {
                return 1 * 1 + 1 * 2 + 3 * 4 + 15 * 8 + 135 * 16 + 2295 * 32 + 75735 * 128;
            }
            if (X < 1 + 2 + 4 + 8 + 16 + 32 + 128 + 512 + 4096) {
                return 1LL * 1LL + 1LL * 2LL + 3LL * 4LL + 15LL * 8LL + 135LL * 16LL + 2295LL * 32LL + 75735LL * 128LL + 9769815LL * 512LL;
            }
            return 1LL * 1LL + 1LL * 2LL + 3LL * 4LL + 15LL * 8LL + 135LL * 16LL + 2295LL * 32LL + 75735LL * 128LL + 9769815LL * 512LL + 5011915095LL * 4096LL;
        }

        [[nodiscard]] bool IsHead(std::size_t X) const {
            return X == 2 || X == 6 || X == 14 || X == 30 || X == 62 || X == 190 || X == 702 || X == 4798;
        }
        ...
        void Merge(TowerHeap_t<Element_t> Tmp, std::size_t Number) {
            ...
            for (std::size_t Index = 1; Index <= TmpHeadSize; Index++) {
                ...
                Tmp.extract();
                for (std::size_t SecondIndex = 0; SecondIndex < sizeof(Element_t) / 8; SecondIndex++) {
                    NewHead[Index * sizeof(Element_t) / 8 + SecondIndex] = Parsed[SecondIndex];
                }
            }
            uint64_t Hole{};
            ...
            while (NewTowerDiskHeight > 0) {
                ...
                for (std::size_t Index = 1; Index <= Tower_t::BlockCapacity(); Index++) {
                    ...
                    Tmp.extract();
                    for (std::size_t SecondIndex = 0; SecondIndex < sizeof(Element_t) / 8; SecondIndex++) {
                        ToDisk[Index * sizeof(Element_t) / 8 + SecondIndex] = Parsed[SecondIndex];
                    }
                }
